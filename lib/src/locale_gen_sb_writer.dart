...
class LocaleGenSbWriter {
  ...
  static String createLocalizationKeysFile(LocaleGenParams params, Map<String, dynamic> defaultTranslations, Map<String, Map<String, dynamic>> allTranslations) {
  ...
  static String createLocalizationFile(LocaleGenParams params, Map<String, dynamic> defaultTranslations, Map<String, Map<String, dynamic>> allTranslations) {
    final hasPlurals = defaultTranslations.values.any((element) => element is Map<String, dynamic>);
    ...
    sb
      ...
      ..writeln('      final overrideLocalizations = await localizationOverrides.getOverriddenLocalizations(locale);')
      ..writeln('      localizations._localisedOverrideValues = overrideLocalizations;')
      ..writeln('    }')
      ..writeln("    final jsonContent = await (bundle ?? rootBundle).loadString('${params.assetsDir}\${locale.toLanguageTag()}.json', cache: useCaching);")
      ...
      ..writeln('    try {')
      ..writeln('      final value = (_localisedOverrideValues[key] ?? _localisedValues[key]) as String?;')
    ...
    if (hasPlurals) {
      sb
        ..writeln('  String _plural(String key, {required num count, List<dynamic>? args}) {')
        ..writeln('    try {')
        ..writeln('      final value = (_localisedOverrideValues[key] ?? _localisedValues[key]) as Map<String, dynamic>?;')
        ..writeln('      if (value == null) return key;')
        ..writeln('      ')
    ...
    sb
      ..writeln('  String getTranslation(String key, {List<dynamic>? args}) => _t(key, args: args ?? <dynamic>[]);')
      ..writeln()
      ..writeln('}');
    return sb.toString();
  }
  ...
  static String createLocalizationDelegateFile(LocaleGenParams params) {
    ...
    params.languages.moveToFirstIndex(params.defaultLanguage).forEach((language) => sb.writeln(LocaleGenParser.parseSupportedLocale(language)));
    sb
      ..writeln('  ];')
      ..writeln()
      ...
      ..writeln('    return _supportedLocales.where((element) => localeFilter?.call(element.toLanguageTag()) ?? true).toList();')
      ..writeln('  }')
      ..writeln()
      ..writeln('  LocalizationOverrides? localizationOverrides;')
      ...
      ..writeln()
      ..writeln('  @override')
      ..writeln('  bool isSupported(Locale locale) => supportedLanguages.contains(locale.toLanguageTag());')
      ..writeln()
      ..writeln('  @override')
  ...
  static String createLocalizationOverrides(LocaleGenParams params) {
    final sb = StringBuffer()
      ...
      ..writeln()
      ..writeln('  Future<Map<String, dynamic>> getOverriddenLocalizations(Locale locale);')
      ..writeln('}');
    return sb.toString();
  }
}
