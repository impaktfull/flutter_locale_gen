...
class TranslationWriter {
  ...
  static void buildTranslationFunction(StringBuffer sb, String key, dynamic value) {
    ...
    try {
      ...
      if (value is Map<String, dynamic>) {
        ...
        arguments = {};
        plural.zero?.let((value) => arguments.addAll(_extractParameters(key: key, value: value)));
        plural.one?.let((value) => arguments.addAll(_extractParameters(key: key, value: value)));
        plural.two?.let((value) => arguments.addAll(_extractParameters(key: key, value: value)));
        plural.few?.let((value) => arguments.addAll(_extractParameters(key: key, value: value)));
        plural.many?.let((value) => arguments.addAll(_extractParameters(key: key, value: value)));
  ...
  static Map<int, String> _extractParameters({required String key, required String value}) {
    ...
    if (allPositionalMatched.isNotEmpty && allNormalMatched.isNotEmpty) {
      throw Exception('The translation for key "$key" contains both positional and normal format parameters');
    }
  ...
  static Map<int, String> _extractPositionalParameters(String key, Iterable<RegExpMatch> matches) {
    // Validate
    ...
    final entries = validMatcher.map((match) => MapEntry(int.parse(match.group(REGEX_INDEX_GROUP_INDEX)!), match.group(REGEX_TYPE_GROUP_INDEX)!)).toList()
      ..sort((a, b) => a.key.compareTo(b.key));

    return Map.fromEntries(entries);
  }

  static Map<int, String> _extractNonPositionalParameters(Iterable<RegExpMatch> matches) {
    var index = 1;
    final entries = matches.map((match) => MapEntry(index++, match.group(NORMAL_REGEX_TYPE_GROUP_INDEX)!));
    return Map.fromEntries(entries);
  }

  static void _buildDefaultPluralFunction(StringBuffer sb, String key, Plural plural) {
    final camelKey = CaseUtil.getCamelcase(key);
    sb
      ..writeln('  String $camelKey(num count) => _plural(LocalizationKeys.$camelKey, count: count);')
      ..writeln();
  }

  static void _buildParameterizedPluralFunction(StringBuffer sb, String key, Plural plural, Map<int, String> indexToReplacement) {
    try {
      final camelKey = CaseUtil.getCamelcase(key);
      ...
      tmpSb.write(') => _plural(LocalizationKeys.$camelKey, count: count, args: <dynamic>[');
      iterationIndex = 0;
      indexToReplacement.forEach((index, match) {
        if (iterationIndex++ != 0) {
          tmpSb.write(', ');
  ...
  static void _buildParameterizedFunction(StringBuffer sb, String key, Map<int, String> indexToReplacement) {
    try {
      final camelKey = CaseUtil.getCamelcase(key);
  ...
  static String _getArgument(String key, String type, int index) {
    ...
    throw Exception('Unsupported argument type for $key. Supported types are -> s,d,f. Create a github ticket for support -> https://github.com/vanlooverenkoen/locale_gen/issues');
  }

  static void _buildDefaultFunction(StringBuffer sb, String key) {
    final camelCaseKey = CaseUtil.getCamelcase(key);
    sb
      ..writeln('  String get $camelCaseKey => _t(LocalizationKeys.$camelCaseKey);')
      ..writeln();
  }

  static void buildDocumentation(StringBuffer sb, String key, Map<String, Map<String, dynamic>> translations, List<String> includedLanguages) {
    ...
    includedLanguages.forEach((language) {
      ...
      var formattedNewLines = value?.toString().replaceAll('\n', '\\n').replaceAll('\r', '\\r');
